...
function injectSocialSections() {
  ...
  const icons = {
    ...
    courses: `<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 3L1 9L12 15L23 9L12 3Z" stroke="#800080" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path d="M21 12L12 17L3 12" stroke="#800080" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path d="M3 16L12 21L21 16" stroke="#800080" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>`
  };
  ...
  const socialLinks = [
    ...
    { name: 'كورسات تعليمية', url: 'https://MG-concrete.nzmly.com', class: 'courses', icon: icons.courses }
  ];

  const socialGridHTML = socialLinks.map(link => `
    <a href="${link.url}" target="_blank" rel="noopener" class="social-card ${link.class}-card">
      <span class="social-icon ${link.class}-icon">${link.icon}</span>
